  ...
  return (
    <>
      <button
        ...
        className="fixed top-4 right-4 z-[201] p-2 directory-sidebar rounded-md md:hidden"
      >
        {isMobileMenuOpen ? <X size={20} /> : <Share2 size={20} />}
      </button>
      <div
        className={`fixed top-0 right-0 md:p-3 lg:p-4 md:bg-background-transparent-color md:backdrop-blur-2xl rounded-bl-sm md:rounded-bl-lg z-[200] 
        ${
          isMobileMenuOpen
            ? "w-screen h-screen bg-[var(--background)] py-16"
            : "hidden"
        } md:block md:w-auto md:h-auto md:py-4`}
      >
        <div
          className={`flex ${
            isMobileMenuOpen ? "flex-col items-center" : "flex-row"
          } md:flex-row content-start justify-end gap-4 text-[12px] md:text-[14px]`}
        >
          {links.map((link) => (
            <Link
              href={link.url}
